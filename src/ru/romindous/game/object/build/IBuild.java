...
public class IBuild implements Build {

	public static final float SPIRE_DMG = 12f;

	private static final int GOLDi = 64;
	private static final int DUSTi = 32;
	...
	public IBuild(final Nexus tm, final WXYZ cLoc, final BuildType bt, final int lvl) {
		this.tm = tm;
		this.cLoc = cLoc;
		this.type = bt;
		this.lvl = (byte) lvl;
	...
	public boolean act() {
		...
		case BARRACK:
			...
			switch (lvl) {
			case 1:
				spawnMob(loc, EntityType.VINDICATOR, "Поберник ур.1", Material.WOODEN_SWORD, null, null, null, null, null);
				break;
			case 2:
				spawnMob(loc, EntityType.VINDICATOR, "Поберник ур.2", Material.STONE_SWORD, null, null, null, null, null);
				break;
			case 3:
				spawnMob(loc, EntityType.VINDICATOR, "Поберник ур.3", Material.IRON_SWORD, null, null, null, null, null);
				break;
			default:
				return false;
			}
		...
		case RANGE:
			...
			spawnMob(loc, EntityType.PILLAGER, "Розбiйник ур." + String.valueOf(lvl), Material.CROSSBOW, null, null, null, null, null);
			break;
		case ALTAR:
			Build.animateAct(loc, 40, 0.4d, Sound.ENTITY_VILLAGER_WORK_CLERIC, 0.6f, Material.BREWING_STAND.createBlockData(), 3);
			spawnMob(loc, EntityType.EVOKER, "Чародiй ур." + String.valueOf(lvl), null, null, null, null, null, null);
			break;
		case CAMP:
			Build.animateAct(loc, 40, 0.4d, Sound.ENTITY_VILLAGER_WORK_WEAPONSMITH, 0.6f, Material.SMITHING_TABLE.createBlockData(), 3);
			switch (lvl) {
			case 1:
				spawnMob(loc, EntityType.ILLUSIONER, "Иллюзор ур.1", Material.WOODEN_AXE, null, null, null, null, null);
				break;
			case 2:
				spawnMob(loc, EntityType.ILLUSIONER, "Иллюзор ур.2", Material.STONE_AXE, null, null, null, null, null);
				break;
			case 3:
				spawnMob(loc, EntityType.ILLUSIONER, "Иллюзор ур.3", Material.IRON_AXE, null, null, null, null, null);
				break;
			default:
				return false;
			}
		...
		case TRIBUNE:
			...
			spawnMob(loc, EntityType.RAVAGER, "Разоритель ур." + String.valueOf(lvl), null, null, null, null, null, null);
			break;
		case UPGRADE:
		...
		case SPIRE:
			...
			if (le != null) {
				loc.getWorld().spawnParticle(Particle.CRIT, loc, 40, 0.4d, 0.4d, 0.4d, 0.2d);
				loc.getWorld().playSound(loc, Sound.ITEM_CROSSBOW_SHOOT, 2f, 0.6f);
				final Vector vc = le.getEyeLocation().subtract(loc).toVector().normalize();
				final SpectralArrow spa = loc.getWorld().spawn(loc.add(vc.multiply(2d)), SpectralArrow.class);
				spa.setShooter(tm.rs.getEntity()); spa.setGravity(false);
				spa.setPierceLevel(lvl); spa.setVelocity(vc);
			}
			break;
		}
		return true;
	...
	public int hp() {
		...
		case ALTAR -> 2 * lvl + 6;
		case CAMP -> 1 * lvl + 14;
		case TRIBUNE -> 4 * lvl + 20;
		default -> 10;};
	}
	
	public float ar() {
		return switch (type) {
		case BARRACK -> 0.01f * lvl + 0.10f;
		case RANGE -> 0.02f * lvl + 0.06f;
		case ALTAR -> 0.01f * lvl + 0.04f;
		case CAMP -> 0.01f * lvl + 0.06f;
		case TRIBUNE -> 0.02f * lvl + 0.12f;
		default -> 0f;};
	}
	
	public float spd() {
		return switch (type) {
		case BARRACK -> 0.01f * lvl + 0.94f;
		case RANGE -> 0.02f * lvl + 0.82f;
		case ALTAR -> 0.04f * lvl + 0.40f;
		case CAMP -> 0.04f * lvl + 0.64f;
		case TRIBUNE -> 0.02f * lvl + 0.96f;
		default -> 1.00f;};
	}
	
	public float dmg() {
		return switch (type) {
		case BARRACK -> 0.6f * lvl + 1.2f;
		case RANGE -> 1.0f * lvl + 2.5f;
		case ALTAR -> 0.4f * lvl + 0.8f;
		case CAMP -> 2.0f * lvl + 2.0f;
		case TRIBUNE -> 1.0f * lvl + 2.0f;
		default -> 1.0f;};
	}
	...
	public float cd() {
		...
		case ALTAR -> 0.0f * lvl + 0.2f;
		case CAMP -> 0.1f * lvl + 0.8f;
		case TRIBUNE -> 0.2f * lvl + 0.5f;
		default -> 1.0f;};
	}
	...
	public float kb() {
		...
		case RANGE -> 0.0f * lvl + 0.4f;
		case ALTAR -> 0.1f * lvl + 0.2f;
		case CAMP -> 0.2f * lvl + 0.4f;
		case TRIBUNE -> 0.4f * lvl + 0.4f;
		default -> 0.0f;};
	}
	...
	public ItemStack getInfoItem() {
		...
		switch (type) {
			...
			case DUST:
				...
				break;
			case SPIRE:
				lr.add("§eНаносит " + StringUtil.toSigFigs((float) tm.rs.mdDmg(SPIRE_DMG), (byte) 1) + " 💢 §eвраждебному существу");
				lr.add("§eпри попадании, каждые §c" + cld + " сек");
				break;
			case UPGRADE:
