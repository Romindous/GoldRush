...
public class RaceInv implements InventoryProvider {
	...
	public void init(final Player pl, final InventoryContent its) {
		...
		if (rc == null) {
			its.fill(ClickableItem.empty(new ItemBuilder(Material.HEART_OF_THE_SEA).name(TCUtils.N + "Рандом")
				.addLore((Object) getComp(null, ar)).addEnchant(Enchantment.MENDING).build()));
		} else {
			its.fill(ClickableItem.of(new ItemBuilder(Material.HEART_OF_THE_SEA).name(TCUtils.N + "Рандом")
				.addLore((Object) getComp(null, ar)).addLore(" ").addLore(TCUtils.P + "КЛИК " + TCUtils.N + " - выбрать").build(), e -> {
		...
		for (final RaceType rt : RaceType.values()) {
			if (rc == rt) {
				its.set(rt.ordinal() << 1, ClickableItem.empty(new ItemBuilder(rt.icn).name(rt.clr + rt.name)
					.addLore((Object) getComp(rt, ar)).addEnchant(Enchantment.MENDING).build()));
			} else {
				its.set(rt.ordinal() << 1, ClickableItem.of(new ItemBuilder(rt.icn).name(rt.clr + rt.name)
						.addLore((Object) getComp(rt, ar)).addLore(" ").addLore(TCUtils.P + "КЛИК " + TCUtils.N + " - выбрать").build(), e -> {
	...
	private String[] getComp(final RaceType rc, final Arena ar) {
		...
		for (final Rusher rs : ar.pls) {
			if (rs.race() == rc) rcs.add((rc == null ? TCUtils.N : rc.clr) + "✦ " + TCUtils.N + rs.name());
		}
