...
public class GRCmd implements CommandExecutor, TabCompleter {
	...
	public boolean onCommand(final CommandSender send, final Command cmd, final String label, final String[] args) {
		if (label.equalsIgnoreCase("gr") && send instanceof Player) {
			...
			case "edit":
				if (args.length < 2 || !ApiOstrov.isLocalBuilder(send, true)) {
					p.sendMessage(Main.prf() + "§cНеправельный синтакс комманды, все комманды - §4/gr help");
					return false;
				}
				...
				if (mm == null) {
					...
					p.sendMessage(Main.prf() + "§cВы уже редактируете карту §4" + mm.stp.nm);
					return false;
				}
				break;
			case "setlobby":
				...
				p.sendMessage(Main.prf() + "§6Точка лобби сохранена на " + 
					"(§4" + Main.lobby.x + "§6, §4" + Main.lobby.y + "§6, §4" + Main.lobby.z + "§6)!");
		        try {ars.save(af);} catch (IOException e) {e.printStackTrace();}
				break;
			case "join":
				rs = Rusher.getPlRusher(p.getName(), false);
				if (rs.arena() != null) {
					p.sendMessage(Main.prf() + "§cВы уже на карте, используйте §4/gr leave§c для выхода!");
					return false;
				}
				if (args.length > 1) {
		...
		if (ApiOstrov.hasParty(p) && ApiOstrov.isPartyLeader(p)) {
			for (final String s : ApiOstrov.getPartyPlayers(p)) {
				...
				if (pl != null && pl.getEntityId() != p.getEntityId()) {
					...
					partyJoinMap(Rusher.getPlRusher(pl.getName(), true), pl, ar);
				}
			}
		}
		ar.join(sh);
