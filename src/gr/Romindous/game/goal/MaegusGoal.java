...
public class MaegusGoal implements MobGoal {
    ...
    public void tick() {
    	...
    	if (((tick++) & 3) == 0) {
    		final boolean sometimes = (tick & 127) == 0;
				...
				if (dmgKd == 0) {
					...
					switch (mob.getType()) {
						case ENDERMAN:
							...
							nwTgt.teleport(eyel.add(0d, -2d, 0d));
							break;
						case WITCH:
							mob.launchProjectile(ThrownPotion.class, 
									eyel.getDirection().multiply(atkKb * nx.mobKb))
								.setItem(potItem);
							break;
						case EVOKER:
							...
							Ostrov.sync(() -> {
								...
								final ArrayList<LivingEntity> ts = new ArrayList<>();
								LocUtil.forEntsIn2D(new WXYZ(eyel, false), AGRO_RANGE >> 5, le -> {
									ts.add(le);
									w.spawn(le.getLocation(), EvokerFangs.class).setOwner(mob);
								});
											...
											if (ors.team() != null) {
												if (ors.team().clr == cc) {
													le.setHealth(Math.min(le.getAttribute(Attribute.GENERIC_MAX_HEALTH)
														.getBaseValue(), le.getHealth() + (atkDm * nx.mobDmg)));
												} else {
													mob.attack(le);
												}
											}
