...
public class KillerGoal implements MobGoal {
	
	private static final int GFood = 4;
	private static final int DFood = 1;
    ...
    public KillerGoal(final Mob mob, final Nexus nx, final int maxHp, final float armor, 
    ...
    public void tick() {
    	...
    	if (((tick++) & 3) == 0) {
			...
			if (mob instanceof Vindicator) ((Vindicator) mob).setJohnny(true);
			...
			if (nwTgt != null) {
				...
				if (dst < mob.getWidth() * 0.4f + MELEE_DST) {
					...
					if (dmgKd == 0) {
						...
						mob.attack(nwTgt);
						mob.swingMainHand();
						nwTgt.addPotionEffect(new PotionEffect(PotionEffectType.WITHER, 40, 2, true, true, true));
						nwTgt.setVelocity(eyel.getDirection().setY(0.1d).multiply(atkKb * nx.mobKb));
					}
					return;
				}
